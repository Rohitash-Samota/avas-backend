...
@Entity
@Table(name = "budget_recommendations", indexes = @Index(name = "idx_budget_recommendation_user", columnList = "requestedBy,createdAt"))
class BudgetRecommendationEntity {
    ...
    @Column(nullable = false, precision = 13, scale = 2) private BigDecimal builtUpAreaSqFt;
    @Enumerated(EnumType.STRING) @Column(nullable = false, length = 30) private PriceCategory category;
    @Column(nullable = false, precision = 19, scale = 2) private BigDecimal evidenceUnitPrice;
    @Column(nullable = false, precision = 19, scale = 2) private BigDecimal lowBudget;
    @Column(nullable = false, precision = 19, scale = 2) private BigDecimal recommendedBudget;
    @Column(nullable = false, precision = 19, scale = 2) private BigDecimal highBudget;
    @Column(precision = 19, scale = 2) private BigDecimal totalBudget;
    @Enumerated(EnumType.STRING) @Column(nullable = false, length = 30) private BudgetFit budgetFit;
    @Enumerated(EnumType.STRING) @Column(nullable = false, length = 30) private PriceCategory suggestedCategory;
    @Enumerated(EnumType.STRING) @Column(nullable = false, length = 20) private ConfidenceLevel confidence;
    @Column(nullable = false) private int evidenceCount;
    @Column(nullable = false, length = 80) private String priceSource;
    ...
    @Column(nullable = false, length = 160) private String modelRelease;
    @Column(nullable = false) private long configurationVersion;
    @Column(nullable = false, updatable = false) private Instant createdAt;
    private Boolean accepted;
    @Column(precision = 19, scale = 2) private BigDecimal actualBudget;
    @Column(length = 1000) private String feedbackNote;
    ...
    UUID requestedBy() { return requestedBy; }
    String city() { return city; }
    BigDecimal builtUpAreaSqFt() { return builtUpAreaSqFt; }
    PriceCategory category() { return category; }
    BigDecimal evidenceUnitPrice() { return evidenceUnitPrice; }
    BigDecimal lowBudget() { return lowBudget; }
    BigDecimal recommendedBudget() { return recommendedBudget; }
    BigDecimal highBudget() { return highBudget; }
    BigDecimal totalBudget() { return totalBudget; }
    BudgetFit budgetFit() { return budgetFit; }
    PriceCategory suggestedCategory() { return suggestedCategory; }
    ConfidenceLevel confidence() { return confidence; }
    int evidenceCount() { return evidenceCount; }
    String priceSource() { return priceSource; }
    List<String> assumptions() { return List.copyOf(assumptions); }
    List<String> explanations() { return List.copyOf(explanations); }
    String modelRelease() { return modelRelease; }
    long configurationVersion() { return configurationVersion; }
    Instant createdAt() { return createdAt; }
    Boolean accepted() { return accepted; }
    BigDecimal actualBudget() { return actualBudget; }
    String feedbackNote() { return feedbackNote; }
    Boolean consentToLearning() { return consentToLearning; }
    Instant feedbackAt() { return feedbackAt; }
}
