...
@Entity
@Table(name = "model_releases", uniqueConstraints = @UniqueConstraint(name = "uk_model_release_name_version", columnNames = {"name", "releaseVersion"}),
        indexes = @Index(name = "idx_model_release_active", columnList = "modelType,status"))
class ModelReleaseEntity {
    @Id private UUID id;
    @Column(nullable = false, length = 120) private String name;
    @Enumerated(EnumType.STRING) @Column(nullable = false, length = 40) private ModelType modelType;
    @Column(nullable = false, length = 60) private String releaseVersion;
    @Column(nullable = false, length = 100) private String provider;
    @Column(length = 120) private String runtime;
    @Column(nullable = false, precision = 5, scale = 4) private BigDecimal validationScore;
    @Enumerated(EnumType.STRING) @Column(nullable = false, length = 20) private ModelStatus status;
    ...
    void validate(BigDecimal score, String note) {
        if (status != ModelStatus.DRAFT) throw new IllegalStateException("Only a draft model can be validated");
        validationScore = score.setScale(4, java.math.RoundingMode.HALF_UP);
        status = ModelStatus.VALIDATED;
        if (note != null && !note.isBlank()) notes = (notes == null ? "" : notes + "\n") + "Validation: " + note.trim();
    }

    void retire() { if (status == ModelStatus.ACTIVE) status = ModelStatus.RETIRED; }
    private static String clean(String value) { return value == null || value.isBlank() ? null : value.trim(); }
    UUID id() { return id; }
    String name() { return name; }
    ModelType modelType() { return modelType; }
    String releaseVersion() { return releaseVersion; }
    String provider() { return provider; }
    String runtime() { return runtime; }
    BigDecimal validationScore() { return validationScore; }
    ModelStatus status() { return status; }
    String notes() { return notes; }
    UUID createdBy() { return createdBy; }
    Instant createdAt() { return createdAt; }
    UUID activatedBy() { return activatedBy; }
